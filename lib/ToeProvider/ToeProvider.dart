...
class _MainDashboardState extends State<MainDashboard> {
  ...
  // EMBEDDED SIMPLE LOGOUT METHOD
  Future<void> _embeddedLogout() async {
    try {
      print('🔓 Starting embedded logout...');
      
      // 1. Clear SharedPreferences directly
      ...
      print('✅ Embedded logout completed');
    } catch (e) {
      print('❌ Embedded logout error: $e');
    }
  }

  // ULTRA SIMPLE LOGOUT - NO ERRORS
  void _handleLogout() {
    print('🚪 Logout button clicked...');
    
    // 1. FIRST navigate immediately (most important)
    Navigator.of(context).pushAndRemoveUntil(
      MaterialPageRoute(builder: (context) => TowProviderLoginPage()),
      (route) => false,
    );
    
    // 2. THEN clear data in background (don't wait)
    _embeddedLogout().catchError((e) {
      print('⚠️ Background logout error (ignored): $e');
    });
  }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              _handleLogout(); // Start logout process
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.red,
            ),
  ...
  void _showAppSettings(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            Icon(Icons.settings, color: Color(0xFF7E57C2)),
            SizedBox(width: 8),
            Text('App Settings'),
          ],
        ),
        content: Text('App settings feature will be implemented in the next update.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: Text('OK'),
                    ...
                    _buildDrawerSection('ACCOUNT', [
                      ...
                      _buildDrawerItem(
                        ...
                        onTap: () {
                          ...
                          _showLogoutConfirmation(context); // Then show confirmation
                        },
                        screenWidth: screenWidth,
                      ),
                    ]),
    ...
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(
          title,
          style: TextStyle(
            color: isLogout ? Color(0xFFF44336) : Color(0xFF333333),
            fontWeight: isSelected ? FontWeight.w600 : FontWeight.normal,
            fontSize: screenWidth > 600 ? 16 : 14,
          ),
        ),
        trailing: badgeCount != null && badgeCount > 0
            ...
            : Icon(
                Icons.chevron_right,
                color: isLogout ? Color(0xFFF44336) : Colors.grey[600],
                size: screenWidth > 600 ? 20 : 18,
              ),
        onTap: onTap,
