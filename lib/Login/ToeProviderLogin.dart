...
class _TowProviderLoginPageState extends State<TowProviderLoginPage> {
  ...
  Future<void> _checkLoginStatus() async {
    try {
      ...
      if (isLoggedIn && userRole == 'tow_provider' && mounted) {
        // Auto-navigate to tow provider dashboard if already logged in
        Navigator.of(context).pushAndRemoveUntil(
          MaterialPageRoute(builder: (context) => MainDashboard()),
          (route) => false,
  ...
  void _login() async {
    ...
    try {
      ...
      // Save login data to SharedPreferences using AuthService
      await AuthService.saveLoginData(
        email: userCredential.user!.email!,
        role: 'tow_provider',
        userId: userCredential.user!.uid,
        userName: userCredential.user!.displayName ?? 'Tow Provider',
      );

      // Also save to TowProviderAuth for role-specific checks
      await TowProviderAuth.saveTowProviderLogin(userCredential.user!.email!);
                                ...
                                children: [
                                  ...
                                  GestureDetector(
                                    onTap: () {
                                      Navigator.of(context).push(
                                        MaterialPageRoute(
                                          builder: (context) => TowProviderRegistrationPage(),
                                        ),
                                      );
                                    },
                                    child: Text(
