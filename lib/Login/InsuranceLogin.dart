...
class _InsuranceLoginPageState extends State<InsuranceLoginPage> {
  ...
  Future<void> _checkLoginStatus() async {
    try {
      ...
      // Only auto-navigate if user is logged in as insurance
      if (isLoggedIn && userRole == 'insurance' && mounted) {
  ...
  void _login() async {
    ...
    try {
      ...
      // Also save in InsuranceAuth for role-specific access
      await InsuranceAuth.saveInsuranceLogin(userCredential.user!.email!);
  ...
  static Future<bool> isUserLoggedIn() async {
    try {
      return await AuthService.checkValidLogin();
    } catch (e) {
      print('Error checking login status: $e');
      return false;
    }
